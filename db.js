...
const BGYS_DB_SURUM = "2026-08-13-cozum-kelimesi-v14";
(function () {
...
function bgysParseSoruMetniFlexible(text, requireAnswer) {
  ...
  blocks.forEach((block, idx) => {
    ...
    lines.forEach(line => {
      ...
      // "Cevap:", "Yanıt:" VEYA "Çözüm:" ile başlayan satırlar cevap harfini içerir.
      // Harften sonra ek metin varsa ("Çözüm: B. Eğitim programları...") o metin açıklama olarak alınır.
      const cevapMatch = line.match(/^(do[ğg]ru\s+)?(cevap|yan[ıi]t|[çc]öz[üu]m|cozum)\s*[:\-]\s*([A-D])\b\s*[\)\.\:\-]?\s*(.*)$/i);
      const acikMatch = line.match(/^(a[çc][ıi]klama)\s*[:\-]\s*(.+)$/i);
      if (cevapMatch) {
        correctLetter = cevapMatch[3].toUpperCase();
        if (cevapMatch[4] && cevapMatch[4].trim()) explanation = cevapMatch[4].trim();
      } else if (acikMatch) {
